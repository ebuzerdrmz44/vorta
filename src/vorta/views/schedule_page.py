...
class SchedulePage(BaseTab, SchedulePageBase, SchedulePageUI):
    def __init__(self, parent=None, profile_provider=None):
        ...
        self.viewmodel = SchedulePageViewModel()
    ...
    def on_scheduler_change(self, _):
        ...
        self.viewmodel.save_schedule(
            profile,
            schedule_mode,
            self.scheduleIntervalUnit.currentData(),
            self.scheduleIntervalCount.value(),
            qtime.hour(),
            qtime.minute(),
        )
    ...
    def populate_from_profile(self):
        data = self.viewmodel.get_schedule_data(self.profile())

        self.schedulerRadioMapping[data['schedule_mode']].setChecked(True)
        self.scheduleIntervalUnit.setCurrentIndex(self.scheduleIntervalUnit.findData(data['schedule_interval_unit']))
        self.scheduleIntervalCount.setValue(data['schedule_interval_count'])
        self.scheduleFixedTime.setTime(QtCore.QTime(data['schedule_fixed_hour'], data['schedule_fixed_minute']))

        self.validationCheckBox.setCheckState(
            QtCore.Qt.CheckState.Checked if data['validation_on'] else QtCore.Qt.CheckState.Unchecked
        )
        self.validationWeeksCount.setValue(data['validation_weeks'])

        self.compactionCheckBox.setCheckState(
            QtCore.Qt.CheckState.Checked if data['compaction_on'] else QtCore.Qt.CheckState.Unchecked
        )
        self.compactionWeeksCount.setValue(data['compaction_weeks'])

        self.pruneCheckBox.setCheckState(
            QtCore.Qt.CheckState.Checked if data['prune_on'] else QtCore.Qt.CheckState.Unchecked
        )
        self.missedBackupsCheckBox.setCheckState(
            QtCore.Qt.CheckState.Checked if data['schedule_make_up_missed'] else QtCore.Qt.CheckState.Unchecked
        )
